...
This script can use the result from HiCCUP directly, instead of
splitting the paired end bam file first.
...
while getopts "hf:n:D:f:B:g:r:e:p:t:R:c:d:o:" OPTION
do
	case $OPTION in
		h)
			usage
...
paired <- pairReads("${name}_read1.bam", "${name}_read2.bam", 
	  sampleName='${name}',DUPLICATETHRESHOLD = ${DUPLICATETHRESHOLD},
	  fileType='${filetype}')
